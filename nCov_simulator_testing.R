...
#  while(sum(status.matrix$infectives, na.rm = TRUE) > 0){ #while there are still infectives
    ...
    if (next.evts=="NewSeeding"){ #print("NewSeeding")
      ...
      events$NewSeeding<-current.time+7
...
#           if (status.matrix$TimeSymptomOnset[person.detected] >= current.time){ 
 ...
 ## GET MODEL OUTPUT ----
  ...
  return(list(time.events=time.events, status.matrix=status.matrix,err=err, n.tests=amountOfTest, school.days.losts=sum(status.matrix$school.days.losts), n.schoolclosure=n.schoolclosure, n.classclosure=n.classclosure, n.detectedpositive=n.detectedpositive))
}
...
register_infected_cases <- function(id.vector,current.time,status.matrix,id.infector){
  ...
  for (i_id in id.vector){
    ...
    status.matrix$recovery.vector[i_id]      <- current.time+nCov.SymptmPeriod(mu.IP = mu.IP) # the total length since infection (Exposed+IP) 
