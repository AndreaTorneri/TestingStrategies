...
total.seeds<-(seeds.ch+seeds.ad)+floor(t.stop/7)*(reseeding.ad+reseeding.ch)
...
for (i_sim in 1:nSim){
  #finalSize[i_sim] <-length(c(which(epi.outbreak[[i_sim]]$time.events[,2]==1),which(epi.outbreak[[i_sim]]$time.events[,2]==2)))
  #finalSize[i_sim] <-length(which(epi.outbreak[[i_sim]]$status.matrix$infected==-1))
